...
-- Best-per-user per mode (public) — one row per (user_id, mode)
-- Guests are kept as separate identities by guest_token prefix.
create or replace view public.leaderboard_best_public as
...
-- Top-5 per user per mode (public; used via /api/me/top5)
create or replace view public.leaderboard_top5_public as
select
  l.created_at,
  l.user_id,
  p.handle,
